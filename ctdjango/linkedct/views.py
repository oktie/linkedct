...
def keyword_search_view(request):
    ...
    if callback:
        q = re.sub('\s', '+', path.get('q', ''))
        fuzzy = path.get('fuzzy', '')
        start = path.get('start', '')
        limit = path.get('limit', '')

        url = url + q + '&fuzzy=' + fuzzy + '&start=' + start + '&limit=' + limit
        ...
        for result in result_list:
            trial = Trial.objects.get(trialid = result['record_id'])
            ...
            all_locations = trial.locations.all().values_list('facility__facility_name', 'facility__address__label')
            locations = []
            ...
            ct_dict[result['record_id']] = [title, sponsor, result['record']['condition_browse'], locations, over20]

        json_dict = {'trials': ct_dict.keys(), 'content': ct_dict}

        response = str(callback) + "(" + json.dumps(json_dict) + ");"
