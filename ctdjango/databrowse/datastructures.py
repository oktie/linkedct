...
class EasyModel(object):
    def __init__(self, site, model):
        ...
        plname = model._meta.verbose_name
        if plname.endswith('s'):
            self.verbose_name_plural = plname + 'es'
        elif plname.endswith('y'):
            self.verbose_name_plural = plname[:-1] + 'ies'
        else:
            self.verbose_name_plural = plname + 's'
...
class EasyInstance(object):
    ...
    def links(self):
        list = []
        if self.instance.__dict__.has_key('is_interlinked'):
            for v in self.instance.interlinks.values():
                resource_id = v['to_resource_id']
                e = External_resource.objects.get(slug=resource_id)
                list.append(e.source_url)
        print list
        return list
    ...
    def related_objects(self):
        ...
        for rel_object in self.model.model._meta.get_all_related_objects() + self.model.model._meta.get_all_related_many_to_many_objects():
            ...
            yield {
                ...
                'related_field_name': rel_object.field.name,
...
class EasyInstanceField(object):
    ...
    def urls(self):
        ...
        if self.field.rel:
            ...
            #
            elif self.field.rel.to._meta.verbose_name == 'external_resource':
                lst = []
                for value in self.values():
